    ...
    <div className="space-y-4">
      ...
      {isSignedIn ? (
        <form onSubmit={onSubmit} className="flex gap-2">
          ...
          />
          <button type="submit" className="btn btn-primary btn-sm btn-square" disabled={createComment.isPending || !content.trim()}>
            {createComment.isPending ? <span className="loading loading-spinner loading-xs" /> : <SendIcon className="size-4" />}
          </button>
        </form>
      ) : (
          ...
          comments.map((comment) => (
            <div key={comment.id} className="chat chat-start">
              ...
              <div className="chat-header text-xs opacity-70 mb-1">
                {comment.user?.name}
                <time className="ml-2 text-xs opacity-50">{new Date(comment.createdAt).toLocaleDateString()}</time>
              </div>
              ...
              {currentUserId === comment.userId && (
                <div className="chat-footer">
                  <button
                    onClick={() => confirm("Delete?") && deleteComment.mutate({ commentId: comment.id })}
                    className="btn btn-ghost btn-xs text-error"
                    disabled={deleteComment.isPending}
                  >
                    {deleteComment.isPending ? <span className="loading loading-spinner loading-xs" /> : <Trash2Icon className="size-3" />}
                  </button>
                </div>
              )}
            </div>
