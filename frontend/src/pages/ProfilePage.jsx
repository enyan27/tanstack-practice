            ...
            <div key={product.id} className="card card-side bg-base-300">
              ...
              <div className="card-body p-4">
                ...
                <div className="card-actions justify-end mt-2">
                  <button onClick={() => navigate(`/product/${product.id}`)} className="btn btn-ghost btn-xs gap-1">
                    <EyeIcon className="size-3" /> View
                  </button>
                  <button onClick={() => navigate(`/edit/${product.id}`)} className="btn btn-ghost btn-xs gap-1">
                    <EditIcon className="size-3" /> Edit
                  </button>
                  <button
                    onClick={() => confirm("Delete this product?") && deleteProduct.mutate(product.id)}
