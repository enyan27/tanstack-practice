...
const ProductPage = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        ...
        {isOwner && (
          <div className="flex gap-2">
            ...
            <button onClick={handleDelete} className="btn btn-error btn-sm gap-1" disabled={deleteProduct.isPending}>
              {deleteProduct.isPending ? <span className="loading loading-spinner loading-xs" /> : <Trash2Icon className="size-4" />}
              Delete
            </button>
          </div>
        )}
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        ...
        <div className="card bg-base-300">
          <figure className="p-4">
            <img src={product.imageUrl} alt={product.title} className="rounded-xl w-full h-80 object-cover" />
          </figure>
        </div>
        <div className="card bg-base-300">
      ...
      <div className="card bg-base-300">
        <div className="card-body">
          <CommentsSection productId={product.id} comments={product.comments} currentUserId={userId} />
        </div>
      </div>
    </div>
  );
